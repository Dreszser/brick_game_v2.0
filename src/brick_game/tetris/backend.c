...
void clear_field(Params_t *prms) {
  for (int i = 0; i < 20 && prms->game_info->field; ++i) {
    for (int j = 0; j < 10; ++j) {
...
void init_next_piece(Params_t *prms) {
  ...
  prms->next_figure->horizontal_coords =
      STARTING_POSITION + (get_x_displacement(prms->next_figure->block_type,
...
void transfer_data_for_printing(Params_t *prms, GameInfo_t *game) {
  if (prms->game_info->field && game->field && prms->game_info->next &&
      game->next) {
    copy_matrix(prms->game_info->field, game->field, 20, 10);
    copy_matrix(prms->game_info->next, game->next, 5, 5);

    int projection_coords = 0;
    if (prms->current_figure->block_type != -1)
      projection_coords = get_lowest_position(prms);
    for (int i = 0; i < 5 && prms->current_figure->block_type != -1; ++i) {
      for (int j = 0; j < 5; ++j) {
        if (get_block_type(prms->current_figure->block_type,
                           prms->current_figure->rotation, i, j)) {
          int y = i + prms->current_figure->vertical_coords - 2;
          int y_projection = i + projection_coords - 2;
          int x = (j * 2 + prms->current_figure->horizontal_coords - 2) / 2;
          if (y_projection >= 0 && game->field[y_projection][x] != 1)
            game->field[y_projection][x] = 3;
          if (y >= 0) game->field[y][x] = 2;
        }
      }
    }
  } else {
  ...
  game->high_score = prms->game_info->high_score;
  game->level = prms->game_info->level;
  game->pause = prms->game_info->pause;
  game->score = prms->game_info->score;
  game->speed = prms->game_info->speed;
}
